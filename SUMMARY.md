# Table of contents

* [Utvecklingsmiljö](README.md)
* [PHP snippets](php-snippets.md)

## Klient & server <a id="kapitel-1"></a>

* [HTML och formulär](kapitel-1/html-och-formulaer.md)

## Intro till PHP <a id="kapitel-2"></a>

* [Grunder i PHP](kapitel-2/att-koda-php.md)
* [Obs! Svenskt språkstöd](kapitel-2/tips-and-tricks.md)

## Projekt

* [En dynamisk webbplats](projekt/en-dynamisk-webbplats.md)
* [Tutorials](projekt/tutorials.md)

## Villkor <a id="kapitel-3"></a>

* [if-satser och loopar](kapitel-3/ifsatser-och-loopar.md)
* [Obs! Formulär till PHP-skript](kapitel-3/skicka-data-fran-formulaer.md)

## Array & loopar <a id="kapitel-4"></a>

* [Arrayer och foreach-loop](kapitel-4/arrayer-och-foreach-loop.md)
* [Labb 1 - slumpa fram ordspråk](kapitel-4/slumpa-fram-ordsprak.md)
* [Labb 2 - omvandla till morse](kapitel-4/labb-omvandla-till-morse.md)
* [Projekt 1 - interaktiv berättelse](kapitel-4/projekt-interaktiv-beraettelse.md)

## Filhantering <a id="kapitel-5"></a>

* [Läsa & skriva filer](kapitel-5/filhantering.md)
* [Labb 3 - läsa csv-fil](kapitel-5/skriva-ut-csv-fil.md)
* [Labb 4 - blogg](kapitel-5/skapa-en-enkel-blogg.md)

## Texthantering <a id="kapitel-6"></a>

* [Arbeta med text](kapitel-6/straenghantering.md)
* [Labb 5 - webscraping](kapitel-6/webscraping.md)
* [Labb 6 - läsa tsv-fil](kapitel-6/skriva-ut-tsv-fil.md)

## Hitta i text <a id="kapitel-7"></a>

* [Regex - reguljära uttryck](kapitel-7/regex-reguljaera-uttryck.md)

## Filhantering 2 <a id="kapitel-8"></a>

* [Skanna katalog](kapitel-8/skanna-kataloger.md)
* [Labb 7 - bildgalleri](kapitel-8/labb-7-bildgalleri.md)
* [Ladda upp filer](kapitel-8/ladda-upp-filer.md)
* [Labb 8 - bygg-din-pc](kapitel-8/labb-7-webbshop.md)

## Använda databas <a id="kapitel-9"></a>

* [Grunder i SQL](kapitel-9/sql-grunder.md)
* [Ställa frågor](kapitel-9/sql-fragor.md)
* [Relationer mellan tabeller](kapitel-9/sql-relationer.md)
* [Inbyggda funktioner](kapitel-9/sql-funktioner.md)
* [Fritextsökning](kapitel-9/sql-fritext.md)
* [Lathund SQL](kapitel-9/lathund.md)
* [Labb 9 - blogg med databas](kapitel-9/labb-9-blogg-med-databas.md)

## Sessioner <a id="kapitel-10"></a>

* [Inloggning och sessioner](kapitel-10/loesenord-and-kryptering.md)
* [Inlogging med Javascript fetch\(\)](kapitel-10/login-modal.md)

## Övrigt

* [Funktioner](oevrigt/funktioner.md)
* [Skicka epost](oevrigt/skicka-epost.md)

